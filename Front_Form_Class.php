...
class Front_Form_Class
{
    ...
    public function email($k,$data,$attr)
    {
        ...
        echo '<div class="form-group" id="'.$data['id'].'">
                <label for="'.$data['type'].'-'.$k.'-'.$key.'">'.$data['label'].'</label>
                <input type="email" class="form-control '.$data['class'].'" id="'.$data['type'].'-'.$k.'-'.$key.'" placeholder="'.$data['placeholder'].'" name="'.$data['type'].'-'.$k.'" '.(($data['required']=='yes')?'required="required"':"").' pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,63}$">
    ...
    public function submit($k,$data,$attr)
    {
        ...
        echo '<img src="'.plugin_dir_url( __FILE__ ).'/assets/images/loading_vcf7.gif" class="loader_gif"/>';
